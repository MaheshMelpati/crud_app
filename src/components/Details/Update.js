...
function Update() {
  ...
  const [state, setState] = useState({
    ...
    JobType: location.state.data.JobType,
    Experience: location.state.data.Experience,
    Salary: location.state.data.Salary,
    JobExpires: location.state.data.JobExpires,
  ...
  return (
    ...
    <div className='w-100 float-start mb-5 border-bottom pb-2'>
      <h3 className='w-50 float-start'>Job Edit Form</h3>
  ...
  <div className="mb-3 ">
    <button className='btn btn-info' onClick={submitData}>UpDate</button>
  </div>
 </div>
 </div>
    </>
